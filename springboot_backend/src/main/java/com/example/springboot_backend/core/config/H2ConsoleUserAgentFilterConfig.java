...
/**
 * H2 控制台请求头过滤器配置 解决 H2 控制台显示 "Sorry, Lynx is not supported yet" 的问题 H2 控制台会检查请求头（User-Agent、Accept
 * 等），如果检测到是文本浏览器或缺少必要的请求头， 就会显示错误。此过滤器为 H2 控制台请求设置标准的浏览器请求头。
 */
@Configuration
...
public class H2ConsoleUserAgentFilterConfig {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(H2ConsoleUserAgentFilterConfig.class);
  private static final String H2_CONSOLE_PATH = "/h2-console";
  private static final String STANDARD_USER_AGENT =
      "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko)"
          + " Chrome/120.0.0.0 Safari/537.36";
  private static final String STANDARD_ACCEPT =
      ...
      if (requestURI != null && fullPath.contains(H2_CONSOLE_PATH)) {
        ...
        HttpServletRequestWrapper wrappedRequest =
            new HttpServletRequestWrapper(httpRequest) {
              ...
              /** 大小写不敏感地查找请求头覆盖 TreeMap 使用 CASE_INSENSITIVE_ORDER，所以直接 get 即可 */
